    ...
    public class TvSeriesNotificationMappingProfile : Profile
    {
        public TvSeriesNotificationMappingProfile()
        {
            ...
            CreateMap<TvSeriasEntity, TvSeriesNotification>()
                ...
                .ForMember(d => d.Title, opt => opt.MapFrom(s => s.TitleRu))
                .ForMember(d => d.SeasonNumber, opt => opt.MapFrom(s => s.ListSeasons.Last().Number))
                .ForMember(d => d.EpisodeNumber, opt => opt.MapFrom(s => s.ListSeasons.Last().ListEpisode.Last().Number))
                .ForMember(d => d.ReleaseDate, opt => opt.MapFrom(s => s.ListSeasons.Last().ListEpisode.Last().ReleaseDateRu));
        }
    }
}