...
namespace Rocket.BL.Tests.ReleaseList
{
	...
	public class MusicDetailedInfoServiceTest
	{
		...
		public void SetUp()
		{
			...
			this._fakeDbMusicData = new FakeDbMusicData(100, 10, MusicCount);

			var mockDbMusicRepository = new Mock<IDbMusicRepository>();
			mockDbMusicRepository.Setup(mock => mock.Get(It.IsAny<Expression<Func<DbMusic, bool>>>(), null, string.Empty))
			...
			this._musicDetailedInfoService = new MusicDetailedInfoService(mockDbMusicUnitOfWork.Object);
		}
		...
		{
			var expectedMusic = this._fakeDbMusicData.Music.Find(f => f.Id == id);

			var actualMusic = this._musicDetailedInfoService.GetMusic(id);
		...
		/// <param name="id">Идентификатор музыкального релиза</param>
		[Test, Order(1)]
		public void GetNotExistedMusicTest([Random(MusicCount, MusicCount + 300, 5)] int id)
		{
			var actualMusic = this._musicDetailedInfoService.GetMusic(id);
		...
		public void AddMusicTest()
		{
			...
			music.Id = this._fakeDbMusicData.Music.Last().Id + 1;

			var actualId = this._musicDetailedInfoService.AddMusic(music);
			var actualMusic = this._musicDetailedInfoService.GetMusic(actualId);
		...
		{
			var music = this._musicDetailedInfoService.GetMusic(id);
			music.Title = new Bogus.Faker().Lorem.Word();

			this._musicDetailedInfoService.UpdateMusic(music);
			var actualMusic = this._fakeDbMusicData.Music.Find(f => f.Id == id);
		...
		{
			this._musicDetailedInfoService.DeleteMusic(id);

			var actualMusic = this._fakeDbMusicData.Music.Find(music => music.Id == id);
		...
		{
			var titleToFind = this._fakeDbMusicData.Music.Find(tv => tv.Id == id).Title;

			var actual = this._musicDetailedInfoService
		...
		[Test, Order(2)]
		public void MusicNotExistsTest([Values("1 1 1", "2 22 2", "", "4 word 4", "three words title")] string title)
		{
			var actual = this._musicDetailedInfoService
